                        ...
                        <table class="shop_table cart">
                            ...
                            <tr class="cart_item">
                                ...
                                <td class="product-remove">
                                    <a href="{{ route('cart.delete',['id'=>$prd->rowId]) }}" class="product-del remove" title="Remove this item">
                                    ...
                                    <div class="cart-product__item">
                                        <a href="#">
                                            <img src="{{ asset($prd->model->image) }}" alt="product" width="150px" height="120px" class="attachment-shop_thumbnail size-shop_thumbnail wp-post-image">
                                        </a>
                                        <div class="cart-product-content">
                                            <h5 class="cart-product-title">{{$prd->name}}</h5>
                                        </div>
                                    </div>
                                </td>

                                <td class="product-price">
                                    <h5 class="price amount">$ {{$prd->price}}</h5>
                                </td>

                                <td class="product-quantity">

                                    <div class="quantity">
                                        <a href="{{ route('cart.decr',['id'=>$prd->rowId, 'qty'=>$prd->qty]) }}" class="quantity-minus">-</a>
                                        <input title="Qty" class="email input-text qty text" type="text" value="{{$prd->qty}}" placeholder="1" readonly>
                                        <a href="{{ route('cart.incr',['id'=>$prd->rowId, 'qty'=>$prd->qty]) }}" class="quantity-plus">+</a>
                                    </div>

                                </td>

                                <td class="product-subtotal">
                                    <h5 class="total amount">$ {{$prd->total()}}</h5>
                                </td>
